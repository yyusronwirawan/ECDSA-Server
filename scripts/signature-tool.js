...
(async () => {
    const message = prompt('Whats the random phrase?\n\n');
    const messageHash = hashMessage(message);
    const privateKey = prompt('Whats your private key?\n\n');
    const signature = await signMessage(messageHash, privateKey);

    console.log(`Your signature is: ${(signature)}`);
    console.log(`Your public key is: ${signatureToPublicKey(message, signature)}`);
})();
